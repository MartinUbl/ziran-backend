...
bool CDatabase_Handler::Connect(const std::string& host, uint16_t port, const std::string& username, const std::string& password, const std::string& dbname) {

	spdlog::info("MySQL client version: {}", mysql_get_client_info());

	mConnection = mysql_init(nullptr);

	if (!mConnection) {
		spdlog::error("Unable to create connection handle");
		return false;
	}
	...
	if (!mysql_real_connect(mConnection, host.c_str(), username.c_str(), password.c_str(), dbname.c_str(), port, nullptr, 0)) {
		spdlog::error("Could not connect to database: {}", mysql_error(mConnection));
		mysql_close(mConnection);
		return false;
	}
...
bool CDatabase_Handler::Init_Statements() {
	...
	auto prepare = [&](const std::string& statement) -> MYSQL_STMT* {
		...
		if (!stmt) {
			spdlog::error("Could not create prepared statement");
			return nullptr;
		}

		int result = mysql_stmt_prepare(stmt, statement.c_str(), static_cast<unsigned long>(statement.size()));
		if (result != 0) {
			spdlog::error("Could not prepare statement: {}: {}", statement, mysql_stmt_error(stmt));
			return nullptr;
		}
...
bool CDatabase_Handler::Load_DB_Plugins() {

	// one-shot query to load plugins
	if (mysql_query(mConnection, "SELECT id, guid FROM plugin")) {
		spdlog::error("Could not execute query: {}", mysql_error(mConnection));
		return false;
	}
	...
	if (result == NULL) {
		spdlog::error("Could not store query result");
		return false;
	}
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (ok) {
			...
			spdlog::error("Invalid GUID in database: {}", guidStr);
		}
	}

	mysql_free_result(result);

	spdlog::info("Loaded {} plugins", pluginCnt);

	return true;
}

bool CDatabase_Handler::Load_DB_Global_Config(CDefault_Environment& env) {
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (!key.empty() && !val.empty()) {
			...
			spdlog::error("Invalid global config entry in database: {}", key);
		}
	}

	mysql_free_result(result);

	spdlog::info("Loaded {} global configurations", cfgCnt);

	return true;
}

bool CDatabase_Handler::Load_DB_Inputs(CDefault_Environment& env) {
	...
	while ((row = mysql_fetch_row(result))) {
		...
		if (!key.empty() && !val.empty()) {
			...
			spdlog::error("Invalid input entry in database: {}", key);
		}
	}

	mysql_free_result(result);

	spdlog::info("Loaded {} input data", cfgCnt);

	return true;
}

bool CDatabase_Handler::Load_DB_Pipelines() {
	...
	spdlog::info("Loaded {} pipelines", pipelineCnt);

	return true;
}

bool CDatabase_Handler::Load_DB_Pipeline_Items() {
	...
	while ((row = mysql_fetch_row(result))) {
		...
		rec.parameters = Parse_Parameters(row[4] ? row[4] : "");
	...
	spdlog::info("Loaded {} pipeline items", pipelineItemCnt);
...
bool CDatabase_Handler::Set_Job_Processed_Timestamp(TJob_Id job_id, time_t timestamp) {
	...
	auto bind_result = mysql_stmt_bind_param(mStmt_Set_Job_Processed_On, param_bind);
	if (bind_result != 0) {
		return false;
	}

	auto result = mysql_stmt_execute(mStmt_Set_Job_Processed_On);
	if (result != 0) {
		return false;
	}
...
bool CDatabase_Handler::Add_Job_Report(TJob_Id job_id, ziran::NJob_Report_Type type, const std::string& value, std::optional<std::string> identifier, std::optional<std::string> extendedHTMLValue) {
	...
	auto bind_result = mysql_stmt_bind_param(mStmt_Add_Job_Report, param_bind);
	if (bind_result != 0) {
		return false;
	}

	auto result = mysql_stmt_execute(mStmt_Add_Job_Report);
	if (result != 0) {
		return false;
	}
...
std::map<std::string, std::string> CDatabase_Handler::Parse_Parameters(const std::string& str) {
	...
	for (auto& c : str) {
		...
		else
		{
			...
			if (c == ' ') {
				...
				spdlog::error("Invalid character in parameters string: '{}'", c);
			}
		}
	}
